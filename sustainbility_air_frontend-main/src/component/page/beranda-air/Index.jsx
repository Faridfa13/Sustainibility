        ...
        <div className="card-body lead p-4">
          {isLoading ? (
            ...
            <div className="table-responsive mt-3" style={{ overflowX: 'auto', whiteSpace: 'wrap' }}>
              <table className="table table-bordered text-center" style={{ tableLayout: 'auto' }}>
                <thead>
                  <tr>
                    <th></th>
                    <th></th>
                ...
                <tbody>
                  <tr>
                    ...
                    {monthlyConsumptionData.map((consumption, index) => (
                      <td key={index}>{parseFloat(consumption).toFixed(2)}m³</td>
                    ))}
                  </tr>
                </tbody>
                <tbody>
                  <tr>
                    ...
                    <td>AKTUAL</td>
                    <td>{ytdConsumptionIndividu}m³</td>{" "}
                    {/* YTD Total Water Consumption/Individu */}
                    {aktualIndividuData.map((item, index) => {
                      const targetValue = targetData[index] || 0; // Mengambil target yang sesuai berdasarkan index
                      const isBelowTarget = item.TotalKonsumsi < targetValue;
                      const isAboveTarget = item.TotalKonsumsi > targetValue;
                        ...
                        <td
                          ...
                          style={{
                            backgroundColor: isBelowTarget
                              ? "lightgreen" // Warna hijau jika di bawah target
                              : isAboveTarget
                              ? "lightcoral" // Warna merah jika di atas target
                              : "white", // Warna default
                          }}
                        >
                          {item.TotalKonsumsi.toFixed(2)}m³
                        </td>
                      );
                    })}
                  </tr>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>AKTUAL</td>
                    <td>{ytdWaterWithdrawalReductionFixed}%</td>{" "}
                    {/* YTD Water Withdrawal Intensity Reduction */}
                    {waterWithdrawalReduction.map((reduction, index) => {
                      const isBelowTarget = reduction > targetReductionValue;
                      const isAboveTarget = reduction < targetReductionValue;
                      ...
                      return (
                        <td
                          ...
                          style={{
                            backgroundColor: isBelowTarget
                              ? "lightcoral" // Warna merah jika di bawah target
                              : isAboveTarget
                              ? "lightgreen" // Warna hijau jika di atas target
                              : "white", // Warna default
                          }}
                        >
                  ...
                  <tr>
                    <td>TARGET</td>
                    <td>
                      {targetReductionValue !== null
                        ? `-${targetReductionValue}%`
                        : "Loading..."}
                    </td>
                    {Array(12)
                      .fill(
                        targetReductionValue !== null ? targetReductionValue : 0
                      )
                      .map((value, index) => (
                        <td key={index}>
